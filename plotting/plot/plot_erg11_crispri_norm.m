function [] = plot_erg11_crispri_norm(strain_to_plot,condition__to_plot,dependency_directory,output_directory)
...
for i=1:length(v_time)
    ...
    for j=1:length(condition_names)
        ...
        %use normalized values to calculate pVals
        to_plot_all=[to_plot_all to_plot];
...
for i=strain_to_plot%1:length(temp_labels1)
    ...
    for j=condition__to_plot
        ...
        xtickangle(45)
        xticklabels(fliplr(temp_labels3))
        %xlim([3.5 10.5])
        xlim([0.5 5.5])
        ylim([0.4 1.1])
