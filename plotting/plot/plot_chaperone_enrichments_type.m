function [] = plot_chaperone_enrichments_type(input_data_switch,list_to_use,...
    variant_type,dependency_directory,output_directory)
...
[~,v_type]=variant_types(mapping_input.variantType);
mapping_input=mapping_input(v_type==variant_type,:);
...
%also filter these by type
[~,v_type]=variant_types(variant_info.variantType);
variant_info=variant_info(v_type==variant_type,:);
...
for i=1:length(v2)
    if v2(i)<0.05
        text(i,2+0.02*i,num2str(v2(i)),'Rotation',45)
    end
end
