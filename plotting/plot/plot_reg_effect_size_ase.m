function [] = plot_reg_effect_size_ase(dependency_directory,output_directory)
...
to_plot{1}=v_delta_delta_z(logical(v_has_ase.*v_buffer.*v_regulatory));
to_plot{2}=v_delta_delta_z(logical(v_has_tag.*~v_has_ase.*v_buffer.*v_regulatory));

to_plot{3}=v_delta_delta_z(logical(v_has_ase.*~v_buffer.*v_regulatory));
to_plot{4}=v_delta_delta_z(logical(v_has_tag.*~v_has_ase.*~v_buffer.*v_regulatory));
