function [] = plot_syn_delta_cai_ase(dependency_directory,output_directory)
...
to_plot{1}=v_delta_cai(logical(v_has_ase.*v_regulatory));
to_plot{2}=v_delta_cai(logical(v_has_tag.*~v_has_ase.*v_regulatory));
...
%to_plot{3}=abs(background_data.variantDeltaCai(v_type_all==2));
