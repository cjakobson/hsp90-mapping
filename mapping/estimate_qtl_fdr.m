...
for n=1:length(filename)
    ...
    load('radFilename.mat')
    toGet=['linear-hsp90/' filename{n} '.mat'];
    if exist(toGet)
        load(toGet)
        trueP{n}=pValues;
    end
    ...
    load('radFilename.mat')
    toGet=['linear-hsp90-perm/' filename{n} '_perm.mat'];
    if exist(toGet)
        load(toGet)
...
for i=1:nProteins
    ...
    %calculate threshold per-protein to constrain FDR
...
save(['pval_cutoffs_from_perm_fdr_' num2str(desiredFDR) '.mat'],'vCutoff')
