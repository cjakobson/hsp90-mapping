...
for q=1:length(conditions)
    ...
    for l=1:length(times)
        ...
        ratioFilename=['linear-hsp90/' times{l} ' ' conditions{q} '_delta.mat'];
        load(ratioFilename)
...
for i=1:length(uniqueQtls)
    
    tempIdx=ismember(toOutput2.index,uniqueQtls(i));
    tempIsEverQtn=logical(sum(toOutput2.isQtn(tempIdx)));
    
    if tempIsEverQtn
        
        toOutput2.isQtn(tempIdx)=1;
...
writetable(toOutput2,'linear_hsp90_fdr_0.05.csv')
